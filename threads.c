...
void disable(int usec){
	...
	timer.it_interval.tv_sec = 200;
	timer.it_interval.tv_usec = usec * 200;

	timer.it_value.tv_sec = 200;
...
int thread_yield() {
    ...
    for(int i = (1 + current_thread_context->number) % MAX_THREADS; i != current_thread_context->number; i ++){
        // loop through all threads following current thread context 
        if(i >= MAX_THREADS) { 
            // loop back through starting from 1
            i = 0;
        }
        // if active thread found
...
void thread_exit(void *return_value) {
    ...
    if(current_thread_context->joiner_thread_number >= 0 && setjmp(current_thread_context->buffer) == 0){
        ...
        //enable preemption // HM: no need: thread_yield does that
        // if (preemption == 1){
        //     set_time(100);
        // }
        thread_yield();
...
/*
* Join threads if the target thread is finished.
*/
